...
export const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onClose }) => {
  const { backendUrl, setBackendUrl, checkConnection } = useCamera();
              ...
              <View className="bg-blue-900/30 border border-blue-700/50 rounded-2xl p-4 mb-6">
                <View className="flex-row items-start">
                  ...
                  <View className="flex-1 ml-3">
                    <Text className="text-blue-300 font-bold text-sm mb-1">Socket.IO Mode</Text>
                    <Text className="text-blue-300/80 text-xs">
                      Real-time streaming via backend server. Works on emulator!
                    </Text>
                  </View>
                </View>
              </View>
              ...
              <View className="mb-6">
                ...
                <View className="mt-3 bg-gray-800 rounded-xl p-3">
                  ...
                  <View className="flex-row flex-wrap gap-2">
                    {Platform.OS === 'android' && (
                      <TouchableOpacity
                        onPress={() => setTempBackendUrl('http://10.0.2.2:3000')}
                    ...
                    <TouchableOpacity
                      onPress={() => setTempBackendUrl('http://localhost:3000')}
                      className="bg-gray-700 px-3 py-2 rounded-lg active:opacity-70"
                    >
                      <Text className="text-blue-400 text-xs font-semibold">Localhost</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      onPress={() => setTempBackendUrl('http://192.168.0.115:3000')}
                      className="bg-gray-700 px-3 py-2 rounded-lg active:opacity-70"
                    >
